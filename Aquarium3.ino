...
// --- sunrise start ---
#define blueCianSunriseHour 6 //Hour for sunrise (0->255) start for blue and cian LEDs
#define whiteSunriseHour 7 //Hour for sunrise (0->255) start for white LEDs
// --- day ---
#define whiteDayHour 8 // Hour for day(255) start for white blue cian LEDs
// --- sunset start ---
#define whiteSunsetHour 12 // Hour for day(255->0) start for white LEDs
#define blueSunsetHour 18 // Hour for day(255->0) start for blue LEDs
#define cianSunsetHour 18 // Hour for day(255->0) start for cian LEDs
// ---- Night ----
#define whiteNightHour 13 //Hour for night(0) start for white LEDs
#define blueNightHour 20 //Hour for night(0) start for blue LEDs
...
void setup()
{
  ...
  // t =  00:00:00 on January 1th 2016
  t.hour = 0;
  t.min = 0;
  t.sec = 0;
  t.year = 2016;
  ...
  oneMoreWhiteLedBarOnPeriodInSeconds = whiteSunriseDurationInSeconds / 5; // So each ... seconds we enable one more led bar connected to relay shield
  ...
  oneMoreWhiteLedBarOffPeriodInSeconds = whiteSunsetDurationInSeconds / 5; // So each ... seconds we disable one more led bar connected to relay shield
...
//    rtc.setTime(19, 11, 00);     // Set the time to 21:58:00 (24hr format)
//    rtc.setDate(23, 5, 2016);   // Set the date to 07 March 2016
}
...
void setLightLevelOnWhitePin(Time currentTime) {
  ...
  if (currentTime.hour >= sunrise && currentTime.hour < day) {
    ...
    ledBarsLightLevel(numberOfLedBar);
  ...
  if (currentTime.hour >= sunset && currentTime.hour < night) {
    ...
    int numberOfLedBar = (int) secondsPassedFromSunset / oneMoreWhiteLedBarOffPeriodInSeconds;
    Serial.print("numberOfLedBar to disable: ");
    Serial.println(numberOfLedBar);
    ledBarsLightLevel(4 - numberOfLedBar);
...
void ledBarsLightLevel(int ledBarsNumber) {
  if (ledBarsNumber == 0) {
    digitalWrite(LED_RELAY_1, HIGH);
    digitalWrite(LED_RELAY_2, HIGH);
    digitalWrite(LED_RELAY_3, HIGH);
